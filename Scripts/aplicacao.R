...
dev.off()
pdf(file = paste(folder,'/figuras/O3max.pdf', sep=''),width = 20, height = 8)
...
#                    header = FALSE)
ONImat <- read.csv(paste(folder,"dados/ONI.csv", sep = ''), sep = ',', header = FALSE)

ONI.f <- rep(ONImat[,1],c(c(31,28,31,30,31,30,31,31,30,31,30,31),
...
dev.off()
pdf(paste(folder,'/figuras/oniano.pdf',sep=''),width = 20, height = 7)
